...
class Repo {
  ...
  /// Обновляет объявление на сервер
  Future<String> updateAds({required AdsModel adsModel}) async {
    final firestore = db.collection(AppConst.collectionData).doc(adsModel.uid);
    try {
      await firestore
          .update(adsModel.toMap())
          .onError((e, _) => 'Error writing document: $e');
      return Status.successful;
    } catch (e) {
      return Status.networkRequestFailed;
